...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updatePositionsAndScaling() {
        ...
        // Ustawiamy pozycję całego kontenera tak, aby nazwa była na wysokości 174 px
        document.querySelector('.faction-info').style.left = `${backgroundLeft + (GUI_WIDTH / 2) * scale}px`;
        document.querySelector('.faction-info').style.top = `${backgroundTop + 174 * scale}px`;
        ...
        document.querySelector('.faction-name').style.fontSize = `${Math.min(80 * scale, 120 * scale)}px`; // Dwukrotnie większa czcionka
        document.querySelector('.faction-name').style.lineHeight = `${110 * scale}px`; // Dopasowanie wysokości linii do wysokości tarczy

        document.querySelector('.faction-ability').style.fontSize = `${Math.min(48 * scale, 72 * scale)}px`; // Dwukrotnie większa czcionka
        document.querySelector('.faction-ability').style.marginTop = `${(276 - 174) * scale}px`; // Pozycja opisu: 276 px - 174 px = 102 px odstępu
