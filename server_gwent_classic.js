...
// Wczytywanie umiejętności królów (liderów)
let isLeaderUsable = () => false;
let executeLeaderEffect = () => ({ action: 'none' });
try {
    let leaderCodeSource = fs.readFileSync(__dirname + '/public/tryby/klasyczny_gwint/krol_code.js', 'utf8');
    // Usuń linie import i bloki export dla czystej interpretacji w new Function
    leaderCodeSource = leaderCodeSource.replace(/^import\s+.*$/gm, '');
    leaderCodeSource = leaderCodeSource.replace(/^export\s*\{[^}]*\};?/gm, '');
    leaderCodeSource += '\nreturn { isLeaderUsable, executeLeaderEffect };';
    const getLeaderCode = new Function('cards', 'calculateEffectivePower', leaderCodeSource);
    const api = getLeaderCode(cards, calculateEffectivePower);
    isLeaderUsable = api.isLeaderUsable;
    executeLeaderEffect = api.executeLeaderEffect;
    console.log(`[CLASSIC SERVER] Loaded krol_code.js successfully`);
} catch (e) {
    console.error('[CLASSIC SERVER] Failed to load krol_code.js', e);
}
...
function initializeGameLeaders(game) {
    if (!game || !game.gameState) return;
    const state = game.gameState;

    // Zainicjuj flagi użycia liderów, jeśli nie istnieją
    if (state.p1Leader === undefined) state.p1Leader = game.player1Leader;
    if (state.p2Leader === undefined) state.p2Leader = game.player2Leader;
    if (state.p1LeaderUsed === undefined) state.p1LeaderUsed = false;
    if (state.p2LeaderUsed === undefined) state.p2LeaderUsed = false;
    if (state.p1LeaderBlocked === undefined) state.p1LeaderBlocked = false;
    if (state.p2LeaderBlocked === undefined) state.p2LeaderBlocked = false;

    // Sprawdź czy któryś z liderów to Emhyr 2003 (blokuje lidera wroga)
    if (String(game.player1Leader) === '2003') {
        state.p2LeaderBlocked = true;
        state.p1LeaderUsed = true; // Emhyr 2003 jest automatycznie zużywany/zagrywany
    }
    if (String(game.player2Leader) === '2003') {
        state.p1LeaderBlocked = true;
        state.p2LeaderUsed = true;
    }
}
    ...
    if (cardIdx !== -1) {
        ...
        if (revivedCardObj && revivedCardObj.moc === 'medyk') {
            ...
            if (reviveable.length > 0) {
                // Czy Emhyr 2005 jest aktywny i to nie jest bohater?
                const isEmhyr2005Active = String(game.player1Leader) === '2005' || String(game.player2Leader) === '2005';
                if (isEmhyr2005Active && !revivedCardObj.bohater) {
                    const randIdx = Math.floor(Math.random() * reviveable.length);
                    const randomCardNumer = reviveable[randIdx];
                    console.log(`[GAME CLASSIC] Emhyr 2005 chain: automatically revived random card ${randomCardNumer}`);
...
function registerClassicGwentEvents(socket, io, games) {
    ...
    socket.on('force-start-game', (data) => {
        ...
        if (games[gameCode]) {
            ...
            if (game.player1FullDeck && game.player2FullDeck) {
                ...
                initializeGameLeaders(game);
                console.log(`[GAME CLASSIC] State initialized with full decks and leaders for ${gameCode}. Waiting for draw request.`);
            }
    ...
    socket.on('request-initial-draw', (data) => {
        ...
        if (game && game.gameState) {
            ...
            // Losujemy 10 kart (lub 11 dla Franceski 3002)
            const hasExtraCard = String(isPlayer1 ? game.player1Leader : game.player2Leader) === '3002';
            const drawCount = hasExtraCard ? 11 : 10;
            let drawn = [];
    ...
    socket.on('play-leader', (data) => {
        const { gameCode, isPlayer1 } = data;
        const game = games[gameCode];
        if (game && game.gameState) {
            const state = game.gameState;
            if (state.currentTurn !== socket.id) {
                console.log(`[GAME CLASSIC] Blocked leader play: Not ${socket.id}'s turn.`);
                return;
            }
            const side = isPlayer1 ? 'p1' : 'p2';
            const leaderNum = isPlayer1 ? game.player1Leader : game.player2Leader;

            // Validate using krol_code.js
            if (!isLeaderUsable(leaderNum, state, isPlayer1)) {
                console.log(`[GAME CLASSIC] Blocked leader play: Leader ${leaderNum} is not usable.`);
                return;
            }

            // Execute using krol_code.js
            const result = executeLeaderEffect(leaderNum, state, isPlayer1, io, gameCode);
            console.log(`[GAME CLASSIC] Player ${isPlayer1 ? 'P1' : 'P2'} used leader ${leaderNum}. Result:`, result);

            if (result.action === 'select_prompt') {
                socket.emit('leader-select-prompt', { type: result.type, cards: result.cards });
                return;
            }

            if (result.action === 'peek_hand') {
                socket.emit('show-peeked-cards', { cards: result.cards });
                result.cards = []; // Nie wysyłaj kart przeciwnikowi w broadcast!
            }

            if (result.action === 'reshuffle_graveyards' || result.action === 'weather') {
                io.to(game.player1).emit('update-deck', { deck: state.p1Deck });
                io.to(game.player2).emit('update-deck', { deck: state.p2Deck });
            }

            // Change turn unless opponent passed
            const oppSide = isPlayer1 ? 'p2' : 'p1';
            if (state[oppSide + 'Passed']) {
                console.log(`[GAME CLASSIC] [${gameCode}] Opponent has passed, turn stays with current player.`);
            } else {
                const nextPlayer = isPlayer1 ? game.player2 : game.player1;
                state.currentTurn = nextPlayer;
            }
    ...
    socket.on('leader-select-choice', (data) => {
        const { gameCode, isPlayer1, choice } = data;
        const game = games[gameCode];
        if (game && game.gameState && game.gameState.leaderSelectPending) {
            const state = game.gameState;
            const pending = state.leaderSelectPending;
            if (pending.isPlayer1 !== isPlayer1) return;

            const side = isPlayer1 ? 'p1' : 'p2';
            const opp = isPlayer1 ? 'p2' : 'p1';
            const leaderNum = isPlayer1 ? game.player1Leader : game.player2Leader;

            state.leaderSelectPending = null;
            let result = { action: 'choice_resolved' };

            if (pending.type === 'opp_graveyard') {
                const oppGrave = isPlayer1 ? state.p2Graveyard : state.p1Graveyard;
                const idx = oppGrave.indexOf(choice);
                if (idx !== -1) {
                    oppGrave.splice(idx, 1);
                    const hand = isPlayer1 ? state.p1Hand : state.p2Hand;
                    hand.push(choice);
                    result = { action: 'take_graveyard_card', card: choice };
                }
            } else if (pending.type === 'weather_deck') {
                const deck = isPlayer1 ? state.p1Deck : state.p2Deck;
                const idx = deck.indexOf(choice);
                if (idx !== -1) {
                    deck.splice(idx, 1);
                    state.board.weather.push(`${side}-${choice}`);
                    result = { action: 'weather', card: choice };
                }
            } else if (pending.type === 'discard_two') {
                // choice should be array of card numbers to discard
                const hand = isPlayer1 ? state.p1Hand : state.p2Hand;
                const deck = isPlayer1 ? state.p1Deck : state.p2Deck;
                const grave = isPlayer1 ? state.p1Graveyard : state.p2Graveyard;

                (choice || []).forEach(num => {
                    const idx = hand.findIndex(c => (typeof c === 'object' ? c.numer : c) === String(num));
                    if (idx !== -1) {
                        const removed = hand.splice(idx, 1)[0];
                        grave.push(typeof removed === 'object' ? removed.numer : removed);
                    }
                });

                const eligibleDeck = deck.map(cNum => cards.find(x => String(x.numer) === String(cNum))).filter(Boolean);
                state.leaderSelectPending = {
                    isPlayer1,
                    type: 'choose_from_deck',
                    cards: eligibleDeck.map(c => c.numer)
                };

                socket.emit('leader-select-prompt', { type: 'choose_from_deck', cards: eligibleDeck.map(c => c.numer) });
                return;
            } else if (pending.type === 'choose_from_deck') {
                const deck = isPlayer1 ? state.p1Deck : state.p2Deck;
                const idx = deck.indexOf(choice);
                if (idx !== -1) {
                    deck.splice(idx, 1);
                    const hand = isPlayer1 ? state.p1Hand : state.p2Hand;
                    hand.push(choice);
                    result = { action: 'take_deck_card', card: choice };
                }
            } else if (pending.type === 'own_graveyard') {
                const grave = isPlayer1 ? state.p1Graveyard : state.p2Graveyard;
                const idx = grave.indexOf(choice);
                if (idx !== -1) {
                    grave.splice(idx, 1);
                    const hand = isPlayer1 ? state.p1Hand : state.p2Hand;
                    hand.push(choice);
            ...
            if (result.action === 'take_deck_card') {
                socket.emit('update-deck', { deck: isPlayer1 ? state.p1Deck : state.p2Deck });
            }

            if (state[opp + 'Passed']) {
                console.log(`[GAME CLASSIC] [${gameCode}] Opponent passed, turn stays.`);
            } else {
                const nextPlayer = isPlayer1 ? game.player2 : game.player1;
                state.currentTurn = nextPlayer;
            }
            ...
            if (cardIdx !== -1) {
                ...
                if (isWeather) {
                    ...
                    if (cardObj.summon) {
                        ...
                        for (let i = hand.length - 1; i >= 0; i--) {
                            ...
                            if (summonIds.includes(String(cNum))) {
                                const sCardObj = cards.find(x => String(x.numer) === String(cNum));
                                const sPos = sCardObj ? (sCardObj.pozycja || 1) : 1;
                                const sTargetRow = `${targetSide}R${sPos}`;
                                if (!state.board[sTargetRow]) state.board[sTargetRow] = [];
                                state.board[sTargetRow].push(cNum);
                                hand.splice(i, 1);
                                musteredCount++;
                                musteredDetails.push({
                                    ...
                                    source: 'hand',
                                    row: sTargetRow,
                                    index: state.board[sTargetRow].length - 1
                                });
                            }
                        }
                        // Z talii
                    ...
                    if (cardObj.moc === 'medyk') {
                        ...
                        if (reviveable.length > 0) {
                            const isEmhyr2005Active = String(game.player1Leader) === '2005' || String(game.player2Leader) === '2005';
                            if (isEmhyr2005Active && !cardObj.bohater) {
                                const randIdx = Math.floor(Math.random() * reviveable.length);
                                const randomCardNumer = reviveable[randIdx];
